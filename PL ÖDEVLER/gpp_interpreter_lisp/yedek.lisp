...
(defun parse-string (input index)
  "String değerleri parse eder (çift tırnak içindeki metinler)"
          ...
          (when (eq (current-token state) 'OP_CP)
            ...
            (let ((body (parse-exp state)))
              ...
              (list 'for var start end body)
                  (push "Missing closing parenthesis in for statement"
                        (parser-state-errors state))))))))))
...
(defun parse-params (state)
  ...
  (let ((params nil))
    ...
    (advance-token state)  ; Skip closing parenthesis
    (nreverse params)))
...
(defun parse-exp (state)
  ...
  (let ((token (current-token state)))
    (cond
      ...
      ((eq token 'OP_QUOTE)  ; liste başlangıcı
       (parse-list state))
      
      ((and (listp token) 
            (eq (first token) 'IDENTIFIER))
       (advance-token state)
       (list 'var (second token)))
...
(defun get-variable (env name)
  ...
  (let ((value (or (gethash name (environment-variables env))
...
;; evaluate-exp fonksiyonunu güncelle
;; evaluate-exp fonksiyonunu güncelleyelim
